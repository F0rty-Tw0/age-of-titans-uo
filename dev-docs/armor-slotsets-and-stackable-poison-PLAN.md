...
- **GM testing commands (2026-07-11): DONE, build green, NOT committed.** `Projects/UOContent/Commands/RarityTestCommands.cs` — `[GenLegendary <id|name>]` (targeted; validation errors explain the right base shape), `[GenVariant <root> <rarity>]` (targeted), `[ClearVariant]`, `[GenArmorSet <material> [rarity=Epic] [root]]` (full set to backpack — capstone testing; default deals the material's 5 thematic roots round-robin, explicit root = uniform set). Loot roller remains the production path (later phase).
