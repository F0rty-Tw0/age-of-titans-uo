...
- **In-game feedback round 1 (2026-07-11): 2 fixes, build + 884/884 green, NOT committed.**
  - 🔴 **Live crash fixed**: merged tick's `AOS.Damage` can kill the target → death runs `Poison = null` → the setter clears the SAME stack list mid-tick → `stacks[0]` after the damage threw IndexOutOfRange (Poison.cs:236). Now re-checks `stacks.Count == 0` right after the damage call and bails (RemoveBuff + Stop). Not unit-testable (needs a firing timer) — needs an in-game re-check: kill something WITH the poison tick itself.
  - FCT stacks: `SetPoisonContext(int stacks)` — merged tick number now floats as "-9 Poison x2" (pre-built labels, zero per-tick allocation). Previously the count was only on the buff bar.
