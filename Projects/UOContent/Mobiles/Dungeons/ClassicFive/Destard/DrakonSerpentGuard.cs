using ModernUO.Serialization;

namespace Server.Mobiles;

// Classic Five bestiary (dev-docs/classic-five-bestiary.md) - Destard, Drakon expansion. L7 core. Donor: OphidianKnight.
[SerializationGenerator(0, false)]
public partial class DrakonSerpentGuard : BaseCreature
{
    [Constructible]
    public DrakonSerpentGuard() : base(AIType.AI_Melee)
    {
        Body = 86;
        Hue = 0x0489;
        BaseSoundID = 634;

        SetStr(585, 625);
        SetDex(155, 175);
        SetInt(60, 85);

        SetHits(655, 675);

        SetDamage(17, 22);

        SetDamageType(ResistanceType.Physical, 100);

        SetResistance(ResistanceType.Physical, 45, 55);
        SetResistance(ResistanceType.Fire, 30, 40);
        SetResistance(ResistanceType.Cold, 35, 45);
        SetResistance(ResistanceType.Poison, 40, 50);
        SetResistance(ResistanceType.Energy, 35, 45);

        SetSkill(SkillName.Swords, 85.0, 100.0);
        SetSkill(SkillName.MagicResist, 80.0, 95.0);
        SetSkill(SkillName.Tactics, 90.0, 100.0);
        SetSkill(SkillName.Wrestling, 85.0, 95.0);

        Fame = 9300;
        Karma = -9300;

        VirtualArmor = 59;
    }

    public override string CorpseName => "a drakon serpent-guard's corpse";
    public override string DefaultName => "a drakon serpent-guard";

    public override int LootBagLevel => 6;

    public override void GenerateLoot()
    {
        AddLoot(LootPack.Average);
    }
}
